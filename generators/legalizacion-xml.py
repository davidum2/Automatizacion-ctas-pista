...
def legalizacionXml(template_path, output_dir, data):
    """
    Crea un documento de legalización de XML basado en una plantilla.
    ...
    try:
        ...
        for paragraph in doc.paragraphs:
            ...
            if '{{RFC_EMISOR}}' in paragraph.text:
                paragraph.text = paragraph.text.replace('{{RFC_EMISOR}}', data['Rfc_emisor'])
            if '{{RFC_RECEPTOR}}' in paragraph.text:
                paragraph.text = paragraph.text.replace('{{RFC_RECEPTOR}}', data['Rfc_receptor'])
        ...
        output_path = os.path.join(output_dir, "legalizacion_xml.docx")
        doc.save(output_path)
        
        return output_path
        
    except Exception as e:
        raise Exception(f"Error al crear legalización de XML: {str(e)}")
