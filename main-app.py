            ...
            for i, partida in enumerate(partidas):
                ...
                if xml_files_in_partida:
                    ...
                    try:
                        ...
                        # Generar documentos para esta factura
                    ...
                    self.update_status(f"  📂 Partida {partida['numero']}: {len(subdirs)} facturas potenciales encontradas.")

                    facturas_en_partida = 0

                    # Procesar cada factura en esta partida
                    for j, subdir in enumerate(subdirs):
                        factura_dir = os.path.join(partida_dir, subdir)
                        self.update_status(f"  📄 Procesando carpeta {j+1}/{len(subdirs)}: {subdir}")

                        # Actualizar la interfaz para mantenerla responsiva
                        self.root.update()

                        # Buscar archivos XML en este directorio de factura
                        xml_files = [f for f in os.listdir(factura_dir)
                                    if f.lower().endswith('.xml') and os.path.isfile(os.path.join(factura_dir, f))]
                    ...
                    self.update_status(f"  ✅ Partida {partida['numero']}: {facturas_en_partida} facturas procesadas.")
